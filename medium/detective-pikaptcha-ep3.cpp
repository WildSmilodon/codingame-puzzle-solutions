...
// v horizontalni smeri se preslika
int flip(int j, int width) {
   int kam = 0;
   if (j <= width/2) {
        kam = width/2 + j;
   } else {
...
int main()
{
    ...
    for (int i = 1; i<height+1; i++) {
        mreza[i][0] = mreza[i][width];
        mreza[i][width+1] = mreza[i][1];
    }
    for (int j= 1; j<width+1; j++) {
        mreza[0][flip(j,width)] = mreza[height][j];
        mreza[height+1][flip(j,width)] = mreza[1][j];
    }
